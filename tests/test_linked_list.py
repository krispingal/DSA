...
class TestLinkedList:
    ...
    @pytest.mark.parametrize("elements, index, expected, msg", [
        ([], 1, None, "Should not be able to access elements from empty LL"),
        ([1], 0, 1, "Should be able to access in a list with only 1 element"),
        ([1, 2, 3], 2, 3, "Should be able to search in a list with multiple elements"),
...
class TestDoublyLinkedList:
    ...
    def test_insert_at_position(self):
        ...
        assert dll.insert_at_position(0, -1), "Should allow insertion in an empty array at idx 0"
        assert dll.head.val == -1, "Inserted element at 0th position should match the value that was passed."
        for i in range(3):
            dll.append(i)
        assert dll.insert_at_position(0, -2)
        assert dll.head.val == -2, "Insert element at 0th position when list is non-empty should pass."
        assert dll.head.next.val == -1, "Previously inserted element should remain there."
        assert dll.insert_at_position(1, -3), "Insert should succeed when position is not sentinel"
        assert dll.head.next.val == -3, "Previously inserted element should remain there."
        assert dll.insert_at_position(6, -4), "Insert should succeed when insert at the end of LL."
        cur = dll.head
        for i in range(6): #LL size is 7, needs 6 skips to reach tail
            cur = cur.next
        assert cur.val == -4, "Inserted value at tail end should be at tail"
        assert not dll.insert_at_position(8, -5), "LL cannot insert without any preceding elements"
